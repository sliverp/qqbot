...
- **群消息支持**：接收、处理和回复 QQ 群消息，支持群消息策略（`groupPolicy`：`open` / `allowlist` / `disabled`）和按群独立配置。
- **群历史消息上下文**：缓存非@消息，被@时注入为对话上下文，支持按群配置 `historyLimit`。
- **群消息门控**：统一三层门控逻辑到 `resolveGroupMessageGate()`，处理 @mention 过滤和消息拦截。
- **群消息合并**：agent 忙碌时自动合并排队消息，附带发送者前缀和去重 mentions。
- **@mention 检测**：`detectWasMentioned` 和 `stripMentionText` 适配器，支持 mentions 列表、事件类型和正则 `mentionPatterns`。
- **按钮交互事件**：`acknowledgeInteraction` API 处理按钮交互事件（type 2001 配置查询、type 2002 配置更新）。
- **按群配置解析**：具体群 → 通配符 `"*"` → 默认值的三级回退链。
- **Agent 级别 `mentionPatterns`**：支持按 agent 配置不同的 @检测关键词。

### 变更

- **消息队列**：从用户维度改为对端维度（`group:{openid}` / `c2c:{openid}`），群聊和私聊独立队列上限。
- **`sendGroupMessage`**：新增 `messageReference` 支持和 `sendAndNotify` 出站钩子。
- **升级脚本**：`upgrade-via-source.sh` 延迟恢复 `channels.qqbot` 配置以兼容 openclaw 3.23+。
