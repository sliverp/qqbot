...
> **在设置任何定时提醒前，必须先确认当前系统时间！**

**为什么需要确认时间？**
- AI 的训练数据可能过时，不知道当前真实日期
- 避免设置错误的提醒时间（如用户说"明天早上8点"，需要知道今天是几号）

**如何确认时间？**
1. **首选**：查看系统消息中的 `【当前系统时间】` 字段
2. **备选**：执行 `date` 命令获取系统时间
...
**何时必须确认时间？**
- 用户使用绝对时间："今天下午3点"、"明天早上8点"、"周五晚上"
- 用户使用相对时间但涉及日期："明天"、"下周一"、"这个月底"
- 用户查询或取消提醒时需要展示时间

**何时可以跳过？**
- 用户使用纯相对时间："5分钟后"、"1小时后"、"半小时后"
- 这些不依赖当前日期，可以直接用 `--at 5m` 等格式
...
> ⚠️ **极其重要**：
> - `--message` 参数**必须**以 `[直接输出]` 开头，这样 AI 会直接发送后面的内容而不是启动对话
> - 完整格式：`"[直接输出] 🔔 {内容}时间到！"`
> - **不要**使用 `--system-prompt` 或 `--system-event`（cron 不支持这些参数）
> - 示例：`--message "[直接输出] 💧 喝水时间到！"`
...
> ⚠️ **`--message` 必须以 `[直接输出]` 开头**，这样 AI 会直接输出后面的内容，不启动对话！
...
**示例**：
- 喝水：`--message "[直接输出] 💧 喝水时间到！"`
- 开会：`--message "[直接输出] 📅 开会时间到！"`
- 打卡：`--message "[直接输出] 🌅 打卡时间到！"`
- 日报：`--message "[直接输出] 📝 写日报时间到！"`

**为什么必须加 `[直接输出]`？**
- 没有这个前缀，AI 会启动初始化对话（询问用户是谁等）
- 加上后，AI 知道只需要输出提醒内容
